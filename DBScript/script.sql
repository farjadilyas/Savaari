...
BEGIN
    ...
    SET MAX_VEHICLE_ID = (SELECT NVL(MAX(VEHICLE_ID),1)
...
VALUES(driverID, 0, 2, 'AEJ248', false, 'Black');
/* --- --- END OF SECTION --- ---*/
...
(
     ...
     FOREIGN_KEY(`DRIVER_ID`) REFERENCES `DRIVER_DETAILS`(`USER_ID`)
) ENGINE = InnoDB; 
...
CREATE OR REPLACE TRIGGER VRR_ON_INSERT
    ...
    FOR EACH ROW
BEGIN
    DECLARE MAX_REGISTRATION_REQ_ID INT;
    SET MAX_REGISTRATION_REQ_ID = 1;
    SET MAX_REGISTRATION_REQ_ID = (SELECT NVL(MAX(REGISTRATION_REQ_ID), 1)
    FROM VEHICLE_REGISTRATION_REQ
    GROUP BY DRIVER_ID
    HAVING DRIVER_ID = NEW.DRIVER_ID);
    
    SET NEW.REGISTRATION_REQ_ID = MAX_REGISTRATION_REQ_ID + 1;
END$$

INSERT INTO VEHICLE_REGISTRATION_REQ
VALUES(driverID, 0, `Honda`, `City`, 2013, `AES209`, `Black`, 0);
